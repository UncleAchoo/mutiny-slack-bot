...
// In-memory cache for processed events (use Redis/database in production)
...
export default async function handler(req, res) {
  ...
  if (type === 'event_callback' && event.type === 'app_mention') {
    ...
    const eventId = `${event.channel}-${event.ts}-${event.event_ts}`;
    ...
    processSlackEvent(channel, ts).catch(console.error);
    return;
  }
...
// Moved main processing logic to separate function
async function processSlackEvent(channel, ts) {
  ...
  const queryAI = async (query) => {
    try {
      ...
      return `Sorry, something went wrong while generating an answer. ${err}`;
    }
  };
  ...
  try {
    ...
    try {
      await postToSlack([{
        type: 'section',
        text: { type: 'mrkdwn', text: '❌ Sorry, I encountered an error processing your request. Please try again or tag @MutinySupport for help.' }
      }]);
    } catch (postErr) {
